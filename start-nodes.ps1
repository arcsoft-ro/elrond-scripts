...
#
Clear-Host
Write-Section "Starting Elrond Node/s"
...
if($result -eq $true){
    if(!$Force.IsPresent){
        ...
        -Message "Valid configuration found. Starting $numberOfNodes $displayAmount!"
    }
}
else{
    if(!$Force.IsPresent){
...
Write-Subsection "Starting nodes"
...
foreach($nodeIndex in $NodeIndexes){
    ...
    if($nodeConfig["ValidKey"] -ne "Yes"){
        Write-DoingAction "Atempting to start service elrond-node-$nodeIndex"
...
#
if($noWarn){
    Write-Section "Successfully started $numberOfNodes $displayAmount nodes"
}
else{
